...
# #########"""
    ...
    def parse_map(data, elf_attack_power=3):
        ...
        for y, line in enumerate(lines):
            ...
            for x, char in enumerate(line):
                if char in ['E', 'G']:
                    attack_power = elf_attack_power if char == 'E' else 3
                    units.append(Unit(x, y, char, attack_power))
                    row.append('.')
                else:
                    row.append(char)
            grid.append(row)
    ...
    def simulate_combat(grid, units, check_elf_deaths=False):
        ...
        while True:
            ...
            for i, unit in enumerate(units):
                ...
                if not targets:
                    ...
                    return rounds, sum(u.hp for u in units), True
                ...
                if target:
                    attack(unit, target)
                    if check_elf_deaths and target.type == 'E' and not target.is_alive():
                        return 0, 0, False

            units[:] = [u for u in units if u.is_alive()]
            rounds += 1

    def find_minimum_elf_attack_power(data):
        elf_attack = 4

        while True:
            grid, units = parse_map(data, elf_attack)
            initial_elf_count = sum(1 for u in units if u.type == 'E')

            rounds, total_hp, elves_won = simulate_combat(grid, units, check_elf_deaths=True)

            if elves_won:
                final_elf_count = sum(1 for u in units if u.type == 'E' and u.is_alive())
                if final_elf_count == initial_elf_count:
                    return elf_attack, rounds, total_hp

            elf_attack += 1

    elf_attack, rounds, total_hp = find_minimum_elf_attack_power(data)
    outcome = rounds * total_hp

    print(f"Elf Attack Power: {elf_attack}, Rounds: {rounds}, Total HP: {total_hp}, Outcome: {outcome}")
