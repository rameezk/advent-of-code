...
def simulate_combat(boss_hp, boss_damage, hard_mode=False):
    spells = {
    ...
    while heap:
        ...
        if hard_mode and state["player_turn"]:
            state = state.copy()
            state["player_hp"] -= 1
            if state["player_hp"] <= 0:
                continue
...
def solve():
    ...
    result = simulate_combat(boss_hp, boss_damage, hard_mode=True)
