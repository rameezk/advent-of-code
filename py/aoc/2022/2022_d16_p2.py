...
# Valve JJ has flow rate=21; tunnel leads to valve II"""
    ...
    def explore(current_valve, time_left, opened_mask, pressure_so_far):
        if opened_mask not in all_states or all_states[opened_mask] < pressure_so_far:
            all_states[opened_mask] = pressure_so_far
        ...
        for valve in important_valves:
            ...
            new_mask = opened_mask | (1 << bit_pos)
            pressure = flow_rates[valve] * time_after_travel
            explore(valve, time_after_travel, new_mask, pressure_so_far + pressure)

    explore('AA', 26, 0, 0)

    max_total = 0
    masks = list(all_states.keys())
    for i in range(len(masks)):
        for j in range(i + 1, len(masks)):
            if masks[i] & masks[j] == 0:
                total = all_states[masks[i]] + all_states[masks[j]]
